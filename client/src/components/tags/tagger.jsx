...
const Tagger = ({ bookmarks }) => {
    ...
    const addedTags = useSelector(state => state.inputs.find(input => input.id === "tagger-add")?.value);
    const removedTags = useSelector(state => state.inputs.find(input => input.id === "tagger-del")?.value);
    ...
    const handleSubmit = async () => {
        ...
        const success = await dispatch(actions.editTags(formData));
