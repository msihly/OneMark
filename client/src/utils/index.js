...
/* ------------------------------- GENERAL ------------------------------- */
export const appendClasses = (original, addendum) => addendum ? (original ? `${original} ${addendum}` : addendum) : original;
...
export const regexEscape = (string) => {
    if (string === undefined) return console.debug("String reference is undefined in regexEscape(...)");
...
export const getArrayDiff = (a, b) => a.filter(e => !b.includes(e)).concat(b.filter(e => !a.includes(e)));
...
export const dateToUTC = (dateTime) => {
    const utcDate = new Date(`${dateTime ?? new Date()} UTC`);
    return !isNaN(utcDate) ? utcDate : new Date(dateTime ?? new Date());
};

export const formatDate = (dateStr, type = "date") => {
    if (!dateStr) return;
    const [year, month, day, rawTime] = [dateStr.substring(0, 4), dateStr.substring(5, 7), dateStr.substring(8, 10), dateStr.substring(11)];
    const date = `${months[+month]} ${day}, ${year}`;
    const time = new Date(`1970-01-01T${rawTime}`).toLocaleTimeString({}, { hour: "numeric", minute: "numeric" });
...
export const getLocalDateTime = (dateStr, { type = "datetime", isDayEnd = false } = {}) => {
    try {
        const dateTime = dateToUTC(isDayEnd ? `${dateStr} 23:59:59` : dateStr).toISOString();
        const [date, time] = [dateTime.substring(0, 10), dateTime.substring(11, 19)];
    ...
    } catch (e) { console.debug(e); return false; }
};
...
export const objectifyFormData = formData => castObjectNumbers(Object.fromEntries(formData.entries()));

export const emptyStringsToNull = obj => Object.fromEntries(Object.entries(obj).map(([key, value]) => [key, value === "" ? null : value]));
...
export const validateForm = ({ reqs = defaultReqs, key, value }) => {
    ...
    else if (r?.maxLen && value !== null && value.length > r.maxLen) errors.push(`${r.name} cannot be greater than ${r.maxLen} characters`);
    else if (r?.minLen && (value === null || value.length < r.minLen)) errors.push(`${r.name} cannot be less than ${r.minLen} characters`);
...
export const validateInput = (value, type) => {
    ...
    switch (type) {
        case "date": case "datetime": {
            if (!/^([0-9]{2,4})-([0-1][0-9])-([0-3][0-9])(?:( [0-2][0-9]):([0-5][0-9]):([0-5][0-9]))?(.[0-9]{1,6})?$/.test(value))
...
export const formatData = (data, type) => {
    if (data === undefined) return console.debug("Undefined reference passed as 'data' argument in formatData(...)");
    if (data === null) return "N/A";

    switch (type) {
        case "boolean": return Boolean(data) ? "Yes" : "No";
        case "currency": return data.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 });
        case "date": case "datetime": case "time": return formatDate(data, type);
        case "integer": return data.toLocaleString("en-US", { maximumFractionDigits: 0 });
        case "percent": return data.toLocaleString("en-US", { style: "percent", minimumFractionDigits: 2, maximumFractionDigits: 2 }).replace("%", "");
        case "number": case "text": default: return data;
    }
};
...
/* ---------------------------------- MATH ---------------------------------- */
export const divide = (...nums) => nums.length > 0 ? (nums.reduce((acc, cur) => acc /= cur) || 0) : null;

export const stringOperators = (operator, firstValue, secondValue) => {
    switch (operator) {
...
/* ------------------------------ RENDERING ------------------------------ */
export const renderArrayString = (element) => element.map?.((e, i) => isValidElement(e) ? cloneElement(e, { key: generateRandomString() }) : e) ?? element;